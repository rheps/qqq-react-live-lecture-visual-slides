...
8. **배경도 살아 있게 (전체화면 레이어)**: 떠다니는 radial-gradient blob 2~3개를 **4:3 캔버스가 아니라 전체화면 `bg-layer`**(app-wrap 안, `position:absolute;inset:0`)에 둔다. blob을 4:3 캔버스(`overflow:hidden`) 안에 넣으면 캔버스 경계에서 잘려 좌우에 빈 공간·경계선이 생긴다. `@keyframes float`로 천천히 움직이고, 다크 테마에서는 더 진하게 빛나도록 색 자동 전환.
9. **지속(ambient) 모션 — 비텍스트에만**: 등장이 끝난 뒤에도 화면이 살아 있도록 `SlideMotion.idle` 프리셋(무한 반복)을 둔다. 강도는 `SlideMotion.K`(기본 2.2, 활발). **단, 지속 모션은 이모지·그림·카드·큰 기호 등 비텍스트 요소에만 적용한다. 제목·핵심문장·질문·본문 같은 읽는 글씨에는 지속 모션을 절대 걸지 않는다**(글씨가 계속 흔들리면 읽기 방해 — 사용자 확정 규칙). 글씨는 `MotionItem`에 `enter`만 주고 `idle`은 주지 않는다. 카드는 인덱스별 delay로 호흡 패턴을 비대칭으로.
...
16. **진행 UI는 viewport 레벨 / 라벨은 Eyebrow**: 진행 막대·페이지 번호는 4:3 캔버스가 아니라 **app-wrap 직속(화면 끝~끝)**에 둔다 — 스케일 안 받고 모니터 전체 기준이 되게. 제목 위 작은 라벨('현재 뉴스' 등)은 길쭉한 pill 칩 금지. **공용 `Eyebrow`는 양옆 줄/알약 박스 대신 "텍스트 + 그라데이션 밑줄"(밑줄형)을 기본으로** 한다(양옆 ——— 줄이나 알약은 촌스럽고 다크에서 묻힘). 전경색 상속으로 라이트=잉크/다크=흰 글씨. 이 밑줄형은 `layoutId` 모핑 앵커로도 최적. 키워드 알약·번호 배지처럼 칩이 맞는 곳은 예외. (`references/motion-theme-upgrades.md` 5번)
...
18. **내장 텍스트 편집기 필수**: 모든 결과물에 브라우저 편집기를 넣는다(HTML·코드 모르는 강사가 직접 수정). `E` 키/우하단 `✏️`로 우측 패널 토글 → 슬라이드 목록 + 모든 텍스트 필드(input/textarea). 편집은 `draftSlides`로 **즉시 화면 반영** + **localStorage 자동저장**(키에 원본 slides 시그니처를 넣어 소스가 바뀌면 옛 편집본 무시). 하단에 **💾 저장**(File System Access로 파일 덮어쓰기, Chrome/Edge·localhost/https; 미지원·`file://`이면 다운로드 폴백) · **⬇ 내보내기**(다운로드) · **↩ 원본으로 되돌리기**. 저장본은 **Babel 주입 컴파일 스크립트 제거 + #root 비움 + doctype**로 재오픈 안전(안 그러면 중복 createRoot로 구버전이 뜸). 편집 패널 input 포커스 중엔 키 네비 무시. **Enter는 줄바꿈으로 반영**(`hl`이 `\n`→`<br/>` 변환 — 3-b). (`references/runtime-and-output.md` 섹션 8)

19. **모션은 요소마다 다르게 + (선택) Magic-Move 모핑**: 등장을 한 종류(스프링 팝)로만 주면 "다 똑같이 띠용 튕긴다"는 단조로움이 생긴다. 프리셋을 여러 개 두고 **요소 종류로 자동 배정**(제목=좌→우 와이프 `reveal`, 부제=`blur`, 캡션=`rise`)하고 **카드 줄은 좌/플립/우로 방향을 번갈아** 들여보낸다(인접 요소가 같은 방향으로 우르르 들어오지 않게). 연결선·화살표는 `pathLength`로 그려지듯. 원하면 거의 모든 슬라이드의 `Eyebrow`에 `layoutId="sectionTag"`를 줘 **슬라이드 간 모핑**(밑줄이 늘어나며 이동)을 켤 수 있다 — 단 모핑을 쓰면 슬라이드 전환은 **변형 없는 크로스페이드**여야 한다(스테이지에 x/scale/rotate 금지, `AnimatePresence`는 기본 sync). 읽는 글씨에 *지속* 모션 금지 규칙은 유지(여기 다양화는 등장 한정). (`references/motion-theme-upgrades.md` 1·2·3번)
...
11. **보라 금지**: indigo/violet/plum 색은 라이트/다크 어디서도 쓰지 않는다. 메인은 블루(#3B82F6), 보조는 sky/teal/emerald/amber/rose. 다크 테마는 순수 블랙.
...
14. **전체화면 배경 — 경계선/빈 공간 없는가**: 4:3 캔버스 바깥(좌우)이 흰색·검정·다른 색으로 비지 않는가? 배경 그라디언트+blob이 모니터 끝까지 이음매 없이 이어지는가? (배경은 viewport 레벨, 캔버스는 투명.)
...
27. **Eyebrow 밑줄형 / 다크=밤하늘**: 제목 위 라벨이 양옆 줄·알약이 아니라 밑줄형인가? 다크가 검정에 가깝고(가장자리) 은하수·별·별똥별이 보이며 중앙 burst는 없는가? (`motion-theme-upgrades.md` 5·6번)
28. **(모핑 사용 시)**: Eyebrow가 슬라이드 넘김에 이어지며 변형되고, 전환이 변형 없는 크로스페이드인가? (`motion-theme-upgrades.md` 2번)
